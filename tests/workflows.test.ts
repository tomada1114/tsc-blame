...
describe("workflow regression checks for repository automation", () => {
  ...
  it("carries no leftovers from the template's bootstrap flow", () => {
    // This repository was generated from a template whose CI had an extra job
    // exercising the bootstrap script. Bootstrap deletes itself along with that
    // job, so a surviving reference names a script that no longer exists — a
    // workflow that fails on every push, for a flow this repository no longer
    // has.
    const source = workflowSource("ci.yml");
    ...
    expect(source).not.toContain("template-only");
  ...
  it("checks the published Node floor with package:smoke after packing", () => {
    const source = workflowSource("ci.yml");
    ...
    // Derived from the manifest rather than named here: the whole point of the
    // job is to consume the tarball on the version `engines.node` promises, so
    // a literal repeated in this file would let the two drift apart silently.
    const floor = /^>=(\d+)$/.exec(manifest.engines?.node ?? "")?.[1];
    expect(floor).toBeTypeOf("string");
    expect(packageFloor).toContain(
      `node-version: ${String(floor)} # published-node-floor`,
    );
...
describe("the development runtime contract fails closed", () => {
  ...
  it("keeps the published floor an explicit bare range", () => {
    // A bare `>=N` is what the package-floor job above parses into a
    // `node-version:`. A caret, an upper bound, or an absent field would leave
    // that job checking a version the package never promised.
